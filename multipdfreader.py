...
EURI_API_KEY="ewzkyNTYsImV4cCI6MTc3NDc3NTI1Nn0.HRHeCucOK0hPVZQwyvNoD0GaHarvHNivjJ2l6-xU1HA"
...
def plot_vectors(vectors, query_vector):
    ...
    fig, ax = plt.subplots()
    # Use explicit RGB colors
    ax.scatter(doc_points[:, 0], doc_points[:, 1], label="Chunks", alpha=0.6)
    ax.scatter(query_point[0], query_point[1], label="Query", marker="x")
    ax.legend()
    st.pyplot(fig)
